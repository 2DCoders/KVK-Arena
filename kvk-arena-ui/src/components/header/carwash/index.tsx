  ...
  return (
    <header className="fixed left-0 top-0 z-50 w-full rounded-full bg-transparent py-2 lg:py-4">
      <div
        className={
          isScrolled
            ? "relative mx-auto flex h-16 max-w-295 items-center justify-between overflow-hidden rounded-full border border-[#31708E]/40 bg-white/75 px-4 py-1.5 shadow-lg backdrop-blur-md lg:h-20 lg:px-8 lg:py-2"
            : "relative mx-auto flex h-16 max-w-295 items-center justify-between overflow-hidden rounded-full border border-white/30 bg-[linear-gradient(135deg,#000000,#111111,#1b2f39)] backdrop-blur-2xl lg:h-20 lg:px-8 lg:py-2"
        }
      >
        {/* glow */}
        <div
          className={
            isScrolled
              ...
              : "pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(49,112,142,0.30),transparent_35%),radial-gradient(circle_at_top_right,rgba(255,255,255,0.10),transparent_30%)]"
          }
        />
        {/* LEFT NAV */}
    ...
    mobileMenuOpen ? "translate-x-0" : "translate-x-full"
        ...
        >
          ...
          <nav className="flex flex-col gap-5">
            ...
            {/* JOIN BUTTON */}
            <a href="#bookings" onClick={closeMobileMenu}>
              <button
                onClick={() => {
                  closeMobileMenu();
                }}
                className="mt-6 cursor-pointer rounded-xl bg-white px-5 py-3 text-sm font-extrabold text-black transition hover:-translate-y-0.5 hover:bg-[#2158bc] hover:shadow-[0_16px_36px_rgba(41,107,225,0.35)]"
              >
                Explore
              </button>
            </a>
