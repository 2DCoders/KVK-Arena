...
public class BookingService : IBookingService
{
    ...
    public async Task<Result> ProcessPaymentSuccessAsync(Guid holdId,CustomerDetails customerDetails, string paymentIntentId, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            // Check if a booking already exists for this payment ID to ensure idempotency
            var existingBooking = await _db.CourtBookings.AnyAsync(b => b.PaymentId == paymentIntentId, ct);
            if (existingBooking)
                return Result.Success("Booking already processed for this payment.");
