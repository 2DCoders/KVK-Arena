...
public class GamingSlotConfigurationService : IGamingSlotConfigurationService
{
    ...
    public async Task<Result> CreateAsync(GamingSlotConfigurationCreateRequest request, CancellationToken cancellationToken = default)
    {
        ...
        if (request.IsActive && await _db.GamingSlotConfigurations.AnyAsync(gsc => gsc.GamingStationId == request.GamingStationId && gsc.IsActive, cancellationToken))
        ...
        try
        {
            ...
            var response = new GamingSlotConfigurationResponse
            {
                ...
                GamingStationId = slotConfiguration.GamingStationId,
    ...
    public async Task<Result> UpdateAsync(GamingSlotConfigurationUpdateRequest request, CancellationToken cancellationToken = default)
    {
        ...
        // Each Gaming Station can have only one active Slot Configuration at a time.
        if (request.IsActive && await _db.GamingSlotConfigurations.AnyAsync(gsc => gsc.Id != request.Id && gsc.GamingStationId == request.GamingStationId && gsc.IsActive, cancellationToken))
        ...
        // Prevent overlapping or conflicting configurations for the same station.
        var conflictingConfig = await _db.GamingSlotConfigurations
            .Where(gsc => gsc.Id != request.Id && gsc.GamingStationId == request.GamingStationId && gsc.IsActive &&
                          ((request.StartTime < gsc.EndTime && request.EndTime > gsc.StartTime) ||
                           (gsc.StartTime < request.EndTime && gsc.EndTime > request.StartTime)))
            .FirstOrDefaultAsync(cancellationToken);
        ...
        try
        {
            existingConfig.GamingStationId = request.GamingStationId;
            ...
            var response = new GamingSlotConfigurationResponse
            {
                ...
                GamingStationId = existingConfig.GamingStationId,
                GamingStationName = gamingStation.Name,
    ...
    public async Task<List<GamingSlotConfigurationResponse>> GetByGamingStationAsync(Guid gamingStationId, CancellationToken cancellationToken = default)
    {
        ...
        var configurations = await _db.GamingSlotConfigurations
            .Where(gsc => gsc.GamingStationId == gamingStationId)
            .Include(gsc => gsc.GamingStation)
            .AsNoTracking()
        ...
        {
            Id = gsc.Id,
            GamingStationId = gsc.GamingStationId,
            GamingStationName = gsc.GamingStation.Name,
    ...
    public async Task<Result> ActivateAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        // Each Gaming Station can have only one active Slot Configuration at a time.
        if (await _db.GamingSlotConfigurations.AnyAsync(gsc => gsc.GamingStationId == configuration.GamingStationId && gsc.IsActive, cancellationToken))
            return Result.Failure($"Gaming Station '{configuration.GamingStationId}' already has an active slot configuration. Deactivate the existing one before activating this configuration.");
