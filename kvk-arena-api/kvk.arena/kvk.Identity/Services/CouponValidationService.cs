...
public class CouponValidationService : ICouponValidationService
{
    ...
    public async Task<CouponValidationResult> ValidateAndCalculateDiscountAsync(string memberId, string couponCode, decimal originalAmount, string moduleName)
    {
        var eligibleOffer = await _context.MemberEligibleOffers
            .Include(x => x.OfferRate)
            .FirstOrDefaultAsync(x => x.CouponCode == couponCode && x.MemberId == GetMemberIdAsync(memberId).Result);
    ...
    public async Task<CouponValidationResult> ValidateAndCalculateDiscountAsync(string couponCode, decimal originalAmount, string moduleName)
    {
         var eligibleOffer = await _context.MemberEligibleOffers
            .Include(x => x.OfferRate)
